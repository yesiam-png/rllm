...
#                           max_num_seqs=self.scheduler_config.max_num_seqs,
    ...
    def from_engine_args(
        ...
        def filter_kwargs_for_init(target_cls, kwargs):
            """Keep only kwargs accepted by target_cls.__init__ (unless it has **kwargs)."""
            sig = inspect.signature(target_cls.__init__)
            params = sig.parameters
            # If __init__ accepts **kwargs, no need to filter
            if any(p.kind == inspect.Parameter.VAR_KEYWORD for p in params.values()):
                return kwargs
            allowed = {k: v for k, v in kwargs.items() if k in params}
            # (Optional) surface what we dropped
            dropped = set(kwargs) - set(allowed)
            if dropped:
                import warnings
                warnings.warn(f"Dropped unsupported config keys: {sorted(dropped)}")
            return allowed
        ...
        cfg_kwargs = config_to_kwargs(engine_config)
        cfg_kwargs = filter_kwargs_for_init(cls, cfg_kwargs)  # <-- drop unknowns like 'prompt_adapter_config'
